...
class FinanceManager {
    ...
    switchTab(tabName) {
        ...
        // Refresh charts if overview tab
        if (tabName === 'overview') {
            setTimeout(() => {
                this.updateCharts();
            }, 100);
    ...
    updateOverview() {
        ...
        const yearlyIncome = this.income
            .filter(i => i.frequency === 'yearly')
            .reduce((sum, i) => sum + i.amount, 0) / 12;
        
        const oneTimeIncome = this.income
    ...
    calculateEstimatedTax(income) {
        ...
        return income * 0.25; // Rough estimate of 25% total tax burden
    }
